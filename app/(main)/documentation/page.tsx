...
`}
			...
			<p>
				...
				under the app folder; <span className="text-primary font-medium">{`(main)`}</span> represents the pages that reside in the main dashboard layout, <span className="text-primary font-medium">{`(full-page)`}</span> groups the pages with full page
				content such as landing page or a login page and <span className="text-primary font-medium">{`(landing)`}</span> is the full page layout that is only available for the dark color scheme.
			</p>
...
@import '../_extensions';`}
			...
			<p>
				Dynamic theming is built-in to the template and implemented by including the theme via a link tag instead of bundling the theme along with a configurator component to switch it. In order to switch your theme dynamically as well, it needs to be
				compiled to css. An example sass command to compile the css would be;
			</p>
			...
			<h4>Migration</h4>
			<p>
				Every important change is included in <b>CHANGELOG.md</b> file at the root folder of the distribution along with the instructions to update. Migration process mainly requires updating the <span className="text-primary font-medium">layout</span>{' '}
				folder and <span className="text-primary font-medium">styles/layout</span> folders.
			</p>
		</div>
	);
};
