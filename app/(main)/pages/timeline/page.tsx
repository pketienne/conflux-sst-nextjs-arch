	...
	const customizedContent = (item: CustomEvent) => {
		return (
			<Card title={item.status} subTitle={item.date}>
				{item.image && <img src={`/demo/images/product/${item.image}`} onError={(e) => ((e.target as HTMLImageElement).src = 'https://www.primefaces.org/wp-content/uploads/2020/05/placeholder.png')} alt={item.name} width={200} className="shadow-2" />}
